...
class InfoCard():
    def __init__(self, txt: TextClass, btn: ButtonClass, Title: str, Description:str, Challenges:str, xcor, ycor):
        self.title = Title
    ...
    def show(self, Screen):
        self.card_rect = pygame.draw.rect(Screen, GREEN, pygame.Rect(self.xcor, self.ycor, self.xcor + self.width, self.ycor+self.height))
        titleText = self.txt(self.title, pygame.font.Font(PoppinsFont, 20), BLACK, (self.xcor, self.ycor))
        titleText.blit()
...
turtle_info = InfoCard(TextClass, ButtonClass, "Leather-Back Sea Turtle", "The largest sea turtle in the world, one that travels thousands of kilometers",
                       "Pollution, Sharks, Orcas", SCREEN_WIDTH_CENTER, SCREEN_HEIGHT_CENTER)
...
bird_info = InfoCard(TextClass, ButtonClass, "Red Winged Blackbird", "A stocky, red and black bird, and one that is very common in North America.",
                     " Air pollution, Hawks, Eagles", SCREEN_WIDTH_CENTER, SCREEN_HEIGHT_CENTER)
